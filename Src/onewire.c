 ...
 *
 * Implementation follows Maxim's tutorial:
 *
...
send_bit(ow_t* ow, uint8_t v) {
    ...
    ow_usart_tr(&v, &b, 1);                     /* Exchange data over USART */
 ...
 * \return          \ref owOK on success, member of \ref ow_res_t otherwise
 */
ow_res_t
ow_lock(ow_t* ow) {
#if OW_USE_RTOS
...
ow_search(ow_t* ow, uint8_t *rom_id) {
    ...
    while (id_bit_number) {
        ...
        while (j--) {
            ...
            /*
             * Devices are expecting master will send 
             * bit value back. All devices which do not have this bit value
             * will go on blocked state till next reset.
             *
